...
class KeyOperationTest {
    ...
    @Test
    void deriveSecretKey() throws Exception {
    ...
    @Test
    void deserializeKeyPair() throws Exception {
        //RSA
        ...
        final PrivateKey rsaDeserializedPrivate = operation.deserializePrivateKey(rsaPrivateKey);
        final PublicKey rsaDeserializedPublic = operation.deserializePublicKey(rsaPublicKey);
        ...
        final PrivateKey ecDeserializedPrivate = operation.deserializePrivateKey(ecPrivateKey);
        final PublicKey ecDeserializedPublic = operation.deserializePublicKey(ecPublicKey);
    ...
    @Test
    void getKeyFingerprint() throws Exception {
        ...
        final Key key = operation.deserializeSecretKey(aesKey);
        final String fingerprint = operation.getKeyFingerprint(key);
        assertEquals(sha256,fingerprint);
    }

    @Test
    void deserializeSecretKey() throws Exception {
        final String aesKey = "D5SZyM4WOrrmp8CUnXID+u6Yr+GXT3xEVa50Pw+/rNs=";
        final Key key = operation.deserializeSecretKey(aesKey);
